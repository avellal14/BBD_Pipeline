...
def xcorr2_fast(T,A):
    # T_size = T.shape
    # A_size = A.shape
    # outsize = np.array(A_size) + np.array(T_size) - 1
    # conv_time = time_conv2(T_size, A_size)
    # fft_time = 3 * time_fft2(outsize)

    # if (conv_time < fft_time):
    #     cross_corr = fftconvolve(np.rot90(np.rot90(T)), A)
    # else:
...
#############################################################
def register_HE2FISH_translation(he,fish):
    H = deconvolveHE(he)
    H = rgb2gray(H)
    H = 255 - H
    ...
    new_he = cv2.warpAffine(he, Transformation, (he.shape[1], he.shape[0]))
