        ...
        for key in ['train','val']:
            if not os.path.isfile(os.path.join(path_dict[key + '_folder'], key + '_log.csv')):
                ...
                for i_image in range(len(list_dict[key + '_image_list'])):
                    ...
                    if(len(groundtruth.shape) > 2 and groundtruth.shape[2] > 1):
                        groundtruth = groundtruth[:,:,1]
 
                    groundtruth[groundtruth==3] = 2 #remove all intra-stromal epithelium labels and set them simply to stroma
                    groundtruth[groundtruth ==4] = 3 #fat label was originally 4 but is now changed to 3
                    ...
                    extractor = extract_patches.sliding_window(dict_obj, flags['size_input_patch'], flags['size_output_patch'],flags['stride'])
                    ...
                    for j, (out_obj_dict, coord_dict) in enumerate(extractor):
                        ...
                        label_name = os.path.join(path_dict[key + '_groundtruth_folder'],
                                                  basename + '_idx' + str(j) + '_row' + str(
                                                          coord_images[0]) + '_col' + str(coord_images[1]) + flags['groundtruth_ext'])
