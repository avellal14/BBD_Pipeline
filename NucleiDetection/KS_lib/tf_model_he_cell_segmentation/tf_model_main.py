...
from KS_lib.tf_model_he_cell_segmentation import tf_model_test
from KS_lib.tf_model_he_cell_segmentation import tf_model_train
from KS_lib.tf_model_he_cell_segmentation import utils
...
def main(nth_fold,mode,flags,testdir):
    ...
    #Segment WSIs
        ...
        list.sort(paths) #crucial step in preventing any complete mess ups(so always goes in numerical order, will allow parallelization)
       # paths = paths[1000:] #TODO: Ignores first 300 images(hoping these can be finished at home)
        print("TEST DIR: " + str(testdir))

        for path in paths:
            print(os.path.join(testdir,path))
            if not os.path.isdir(os.path.join(testdir,path+'cellSeg')): #prevents this from being executed with exsiting directories
                 ...
                 #if(caseControlList[1] == '1'): #only segment cases
                  #      tf_model_test.testWSI(object_folder, model_path, os.path.join(testdir,path), flags)
