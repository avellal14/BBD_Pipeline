...
def applyMask(filename):
   ...
   wsiThmbnl = wsiOG.read_region((0, 0), 8, wsiOG.level_dimensions[8]) #TODO: need to keep same mask size as 40x (289 x 581)
...
def preprocessImage(filename, destination):
   ...
   thumbnailSaved = wsi.read_region((0, 0), 8, wsi.level_dimensions[8]) #again, in 20x make sure that thumbnail is approx. 289 x 581
   ...
   while (i < length):
       j = 0
       while (j < width):
           patch = mask[i: i + 16, j: j + 16] #16 x 16 thumbnail * (2^8, 2^8) = 4096 x 4096 patch @ 20x 
           patchSum = np.sum(patch)
           #if the patch has some tissue, extract it from the whole slide at 40x (size = 2048 x 2048 pixels)
           if (patchSum > 0):
              
              #16 x 16 in thumbnail = 4096 x 4096 @20x --> 16 x 16 thumbnail = 4096 x 4096 @20x = 2048 x 2048 @10x
              #8 x 8 times (256 x 256) --> 2048 x 2048 at 0 zoom out
              #also --> 256 (or 2^[8-0]) is the scale factor btwn thumbnail and actual WSI
               currentPatch = wsi.read_region((j * 256, i * 256), 1, (2048, 2048))
...
def processCSV(csvLocation, destinationDirectory):
    ...
    letterArray = ['A', 'B', 'C',
                   'D']  # this array is simply for processing the image names (some WSIs have A,B,C,D instead of 1,2,3,4)
    ...
    while (i < len(patientList)):
        ...
        if (len(currentPatient[3]) > 0):
            ...
            mrxsFileList = [f for f in mrxsFileList if
                            os.path.isfile(os.path.join(currentPatient[4], f)) and currentPatient[0] in f]
            ...
            for j in range(len(mrxsFileList)):
                ...
                destination = os.path.join(destinationDirectory,
                                           mrxsFileList[j][0:len(mrxsFileList[j]) - 5] + '_patches')
...
processCSV(os.path.join('R:', os.sep, 'Beck Lab', 'HENG_BBD_Texture_NHS', 'BBD_NCC_modifiedWithPaths_tocreatepatches40x_15Mar18.csv'), os.path.join('R:', os.sep, 'Beck Lab', 'HENG_BBD_Texture_NHS', 'BBD_NCC_10xExtraction_20x'))
###################
