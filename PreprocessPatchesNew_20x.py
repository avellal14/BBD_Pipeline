...
def processCSV(csvLocation, destinationDirectory):
   ...
   i = imageStartIndex
   if(imageEndIndex > len(patientList)):
      imageEndIndex = len(patientList)
   
   while(i < imageEndIndex):
       ...
       if(len(currentPatient[3])>0):
           ...
           for j in range(len(mrxsFileList)):
               ...
               if os.path.exists(currentFilePath) and not os.path.exists(destination):
                   if(totalNumImagesProcessed >= imageStartIndex):
                   ...
                   totalNumImagesProcessed = totalNumImagesProcessed + 1

           trueCounter = trueCounter + 1 #add 1 to the number of patients processed
